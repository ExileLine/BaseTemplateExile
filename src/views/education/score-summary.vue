<template>
  <page-container>
    <base-table
      ...
      url="http://0.0.0.0:7766/api/score/upload_history_page"
    >
        ...
        <t-button
          ...
          onClick={() => {
            const alertDia = DialogPlugin.alert({
              header: '内容',
              body: row.analyze_result_content,
              width: '1000px',
              confirmBtn: {
                content: '复制',
              },
...
const actionOptionList = [
  ...
  // },
  {
    ...
    async onClick({ row }) {
      ...
      // window.open('http://0.0.0.0:7766/api/score/downloadSourceZip?uuid=' + row.uuid)
